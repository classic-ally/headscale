...
const (
	...
	setName         = 5
	updateMulti     = 6
	// WireGuard-only peer operations. Renumbered above upstream's setName
	// and updateMulti when v0.29.3 was merged; the values are internal to
	// the write queue and are not persisted anywhere.
	putWGPeer     = 7
	delWGPeer     = 8
	putConnection = 9
	delConnection = 10
...
func NewNodeStore(allNodes types.Nodes, allWGPeers types.WireGuardOnlyPeers, allConnections types.WireGuardConnections, peersFunc PeersFunc, batchSize int, batchTimeout time.Duration) *NodeStore {
	...
	snap := snapshotFromNodes(nodes, peersFunc, nil, wgPeers, connections)
...
func (s *NodeStore) applyBatch(batch []work) {
	...
	prev := s.data.Load()
	newSnap := snapshotFromNodes(nodes, s.peersFunc, prev.routes, wgPeers, connections)
...
//
// wgPeers and connections index the WireGuard-only peers and the per-node
// connections to them. They are part of the same copy-on-write snapshot as
// the node indexes so a reader sees a consistent view of both.
