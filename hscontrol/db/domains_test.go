...
func TestCreateDomain(t *testing.T) {
	runOnBothDialects(t, func(t *testing.T, db *HSDatabase) { //nolint:thelper
...
func TestCreateDomainZone(t *testing.T) {
	runOnBothDialects(t, func(t *testing.T, db *HSDatabase) { //nolint:thelper
		zone, err := db.CreateDomain(types.Domain{
			Domain:   "example.com",
			Provider: new("cloudflare"),
			APIToken: new("cf-token-123"),
		})
		require.NoError(t, err)
...
func TestDeleteDomainZoneWithChildren(t *testing.T) {
	runOnBothDialects(t, func(t *testing.T, db *HSDatabase) { //nolint:thelper
		...
		require.ErrorIs(t, err, ErrZoneHasChildren)
